...
import {AddTenantIdCardScreenProps} from '@models/navigators/HomNavigator';

const AddTenantIdCardScreen: React.FC<AddTenantIdCardScreenProps> = ({
  navigation,
  route,
}) => {
  ...
  const onFinish = async (values: CreateTenantInputControl) => {
    try {
      setLoading(true);
      const identityCard: CreateIdentityCardModel = {
      ...
      await tenantService.createTenantIdCard(
        identityCard,
        roomId || '',
  ...
  return (
    <View style={{...globalStyles.container, rowGap: 8}}>
      <TitleComponent
        ...
        back
        title={t('actions.addIdentityCard')}
      />
            ...
            render={({field}) => (
              <View>
                <InputComponent
                  ...
                  require
                  errorMessage={
                    errors.name ? t('validation.tenantNameRequired') : undefined
                  }
                />
              </View>
            )}
...
export default AddTenantIdCardScreen;
